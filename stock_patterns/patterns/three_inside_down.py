"""Three Inside Down — bullish candle, bearish harami, then confirmation down-close."""
...
def generate(seed: int | None = None, with_context: bool = True) -> pd.DataFrame:
    ...
    c2_close = max(c2_close, c1_open + body1 * 0.05)
    c2 = make_candle(
        open_=c2_open,
        close=c2_close,
    ...
    c3_open = c2_close + body1 * rng.uniform(-0.1, 0.05)
...
def matches(df: pd.DataFrame) -> bool:
    ...
    first_bull = a["close"] > a["open"]
    second_bear = b["close"] < b["open"]
    inside = b["open"] < a["close"] and b["close"] > a["open"]
    confirm = c["close"] < a["open"]
    return bool(first_bull and second_bear and inside and confirm)
